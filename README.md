...
|----------|-------------------|-------------------|
| **Consistent Hashing** | Шардирование мероприятий по городам в Cassandra | Равномерное распределение мероприятий по узлам кластера с минимальным перераспределением при добавлении/удалении узлов. Ключ шардирования: city + event_id |
| **Token Bucket** | Rate limiting API endpoints | Ограничение количества запросов к API (300 запросов/мин на пользователя). Защита от DDoS и злоупотреблений |
| **Cache Aside** | Кэширование данных мероприятий и пользователей | При чтении: сначала кэш Redis, при промахе - БД. При записи: инвалидация кэша + запись в БД |
| **Idempotency Key** | Покупка билетов, платежи | Клиент генерирует уникальный ключ для каждой операции покупки. Повторный запрос с тем же ключом возвращает существующий результат |
| **Two-Phase Commit (2PC)** | Создание заказа + списание билетов | Атомарная транзакция: резервирование билетов → создание заказа → подтверждение платежа → фиксация операции |
| **LRU (Least Recently Used)** | Кэширование в Redis | Вытеснение наименее востребованных данных из кэша при достижении лимита памяти |
| **BM25 + TF-IDF** | Поиск мероприятий в Elasticsearch | Релевантный полнотекстовый поиск с учетом частоты терминов и длины описания мероприятий |
| **Collaborative Filtering** | Рекомендации мероприятий | Рекомендации на основе похожих пользователей: "пользователи, которые смотрели это мероприятие, также смотрели..." |
| **Content-Based Filtering** | Персональные рекомендации | Рекомендации на основе предпочтений пользователя: категории, ценовые диапазоны, локации |
| **Gradient Boosting** | Ранжирование поисковой выдачи | ML-модель для сортировки мероприятий в поиске по релевантности для конкретного пользователя |
